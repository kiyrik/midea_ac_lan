...
- Enable advanced parameters (optional). This turns on additional runtime data polling (e.g. pressures, temperatures, defrosting state) for devices that support it.
...
## Advanced Diagnostics (CLI)

Use the midea-local CLI to validate advanced parameters outside HA.

- Enable advanced params (UNITPARA for C3):

```bash
midealocal discover --host 192.168.0.175 --enable-advanced-params -d
```

- Or pass Customize JSON explicitly:

```bash
midealocal discover --host 192.168.0.175 --customize '{ "enable_advanced_params": true }' -d
```

The logs will include additional attributes such as defrosting status, high/low pressure, T4/T5 temperatures, EXV current, and instant power if your device supports them.
